...
if ( args.variable === undefined ) {
  ...
  if ( containsCSC ) { servicesOfferedArray[servicesOfferedArray.length] = '1816' }
...
const CyclingCadenceService = require('./csc/cycling-cadence-service');
...
class ZwackBLE extends EventEmitter {
	...
	constructor(options) {
		...
		this.csc = new CyclingCadenceService();
	...
	notifyCSC(event) {
		debugCSC(`[${this.name} notifyCSP] ${JSON.stringify(event)}`);
